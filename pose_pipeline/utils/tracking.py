...
def annotate_single_person(filt, subject_id=0, confirm=False):
    ...
    keys = ((TrackingBbox & filt 'num_tracks=1') - pose_pipeline.PersonBboxValid).fetch('KEY')
    ...
    for k in keys:
        ...
        track_id = np.unique([[t['track_id'] for t in t2] for t2 in tracks])
